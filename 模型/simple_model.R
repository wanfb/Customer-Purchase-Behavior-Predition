...
# lhood_list = c("adFlag","availability", "manufacturer",'pid', 
#                'pharmForm',"genericProduct","category",
#               'salesIndex', "day_7","group_12","group_34")
...
model = xgboost(data = as.matrix(train), label = train_labels$order,
                max.depth = 6, eta = 0.3, nthread = 64, nround = 10, 
                subsample = 1,
...
#把模型存下来
save(model,file = "YourModel.RData")
...
# model2 = glm(as.integer(quantity) ~ ., data = model2_data, family = poisson)
# 
# #使用第二层模型预测
# is_test2 = (preds01 == 1)
# test2 = test[is_test2, ]
# price = data.frame(test2)$price
# 
